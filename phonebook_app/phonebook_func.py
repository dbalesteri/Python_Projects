...
def onSelect(self,event):
    ...
    with conn:
        ...
        #This returns a tuple so can slice it into 4 parts using data[] during iteration
        for data in varBody:
...
def onRefresh(self):
    ...
    with conn:
        cursor = conn.cursor()
        cursor.execute("""SELECT COUNT(8) FROM tbl_phonebook""")
        count = cursor.fetchone()[0]
        i = 0
        while i < count:
...
def onUpdate(self):
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0): #ensure there is data present
        ...
        with conn:
            cur = conn.cursor()
            #count records to see if user's changes are already in the db, meaning there are no changes to update
